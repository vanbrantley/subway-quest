...
| `int_trip_signatures` | one row per `(user_id, trip_id, signature)` | A trip's ordered stop sequence (first leg's entry complex, then every leg's exit complex in order), `complex_id`-based — SQL equivalent of `quests_logic.ts`'s `tripSignature()`. Powers `unique_trip_pattern_count` below. |
| `int_quest_completion_counting` *(milestone 8)* | one row per `(user_id, quest_id)`, counting-mechanism quests only | SQL equivalent of `evaluateCounting()`/`evaluateTiers()` — `ride_count_route` (always a real route now, one auto-generated quest per line, no more `'any'`), `transfer_count` (reuses `int_transfers`, see that row's flagged assumption above), `total_ride_count` (lifetime leg count), and `unique_trip_pattern_count` (reuses `int_trip_signatures` above). `completed` = reached the FIRST/lowest tier, not the final one — mirrors the mobile side's tiered semantics. |
